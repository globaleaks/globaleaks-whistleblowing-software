...
class _StreamingEncryptionObject(object):
    ...
    # === Unified padding configuration (messages + streaming v2) ===
    PADDING_FRACTION = 0.05      # 5% default
    PADDING_MIN = 8              # min 8 bytes
    PADDING_MAX = 65536          # max 64KB
    ...
    def _pad_bytes(self, data: bytes) -> bytes:
        """
        Generic padding used for:
          - message-level symmetric encryption
          - streaming v2 chunk padding
        Format:
            data || random_padding || pad_len(2B big-endian)
        """
        data = _convert_to_bytes(data)
        n = len(data)
        max_pad = int(n * self.PADDING_FRACTION)
        if max_pad < self.PADDING_MIN:
            max_pad = self.PADDING_MIN
        if max_pad > self.PADDING_MAX:
            max_pad = self.PADDING_MAX
    ...
    def _strip_padded_bytes(self, data: bytes) -> bytes:
        """Remove unified padding trailer if valid, else return original."""
        if len(data) < 2:
            return data
        pad_len = struct.unpack(">H", data[-2:])[0]
        if pad_len > len(data) - 2:
            return data   # legacy/unpadded
        if pad_len > self.PADDING_MAX:
            return data
    ...
    def _write_header_v1(self) -> None:
        sealed = _GCE.asymmetric_encrypt(self.user_key, self.key)
        self.fd.write(sealed)
    ...
    def _write_header_v2(self) -> None:
        ...
        ss_header = crypto_secretstream_xchacha20poly1305_init_push(self.ss_state, self.key)
        self.fd.write(ss_header)
    ...
    def _read_header_v2(self) -> None:
        ...
        if len(version) != 1:
            raise ValueError("Corrupted v2 header: version")
    ...
    def getNextNonce(self, last: int) -> bytes:
        if last:
            nonce = self.lastFullNonce()
        else:
            nonce = self.fullNonce(self.index)
        self.index += 1
        return nonce

    def _encrypt_chunk_v1(self, chunk: bytes, last: int) -> None:
        """
        v1 streaming: SecretBox chunking, NO padding.
        """
    ...
    def _encrypt_chunk_v2(self, chunk: bytes, last: int) -> None:
        """
        v2 streaming: SecretStream frames WITH per-chunk padding.
        """
        # --- Apply unified padding (Option 2: pad_len only) ---
    ...
    def encrypt_chunk(self, chunk: bytes, last: int = 0) -> None:
        ...
        if self.use_secretstream:
            self._encrypt_chunk_v2(chunk, last)
        else:
            self._encrypt_chunk_v1(chunk, last)
    ...
    def _decrypt_chunk_v1(self) -> Tuple[int, bytes]:
        ...
        chunk_nonce = self.getNextNonce(last)
        plaintext = self.box.decrypt(ct, chunk_nonce)
        return last, plaintext

    def _decrypt_chunk_v2(self) -> Tuple[int, bytes]:
        """
        v2 streaming: SecretStream frames WITH unified padding.
        """
        sz = self.fd.read(4)
        if not sz:
            self.EOF = True
        ...
        last = 1 if tag == crypto_secretstream_xchacha20poly1305_TAG_FINAL else 0
        if last:
            self.EOF = True

        # --- Remove unified padding (Option 2: pad_len only) ---
        msg = self._strip_padded_bytes(msg)

        return last, msg

    def decrypt_chunk(self) -> Tuple[int, bytes]:
        """
        Decrypt one chunk/frame. Returns (last, plaintext).
        """
        if self.use_secretstream:
            return self._decrypt_chunk_v2()
        return self._decrypt_chunk_v1()
