...
def csv_sanitize_cell(value):
    ...
    a formula trigger (=, +, -, @) or a control character (tab/CR) is executed
    as a formula by Excel/LibreOffice on open. The filename is attacker
    controlled, so prefix any such cell with a single quote."""
