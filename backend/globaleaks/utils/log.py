...
def open_log_file(logfile, max_file_size, rotated_log_files):
    """
    Open a log file
...
def log_formatter(timestamp, request):
    """
...
class LogObserver(txlog.FileLogObserver):
    ...
    def emit(self, event_dict):
        ...
        error counters. The event_dict is generated by the arguments passed to each
        log level call. See the unittests for an example.
        """
        if 'failure' in event_dict:
            vf = event_dict['failure']
            e_t, e_v, e_tb = vf.type, vf.value, vf.getTracebackObject()
            sys.excepthook(e_t, e_v, e_tb)

        text = txlog.textFromEventDict(event_dict)
        if text is None:
            return

        time_str = self.formatTime(event_dict['time'])
        fmt_dict = {'system': event_dict['system'], 'text': text.replace("\n", "\n\t")}
        msg_str = txlog._safeFormat("[%(system)s] %(text)s\n", fmt_dict)
