...
def extract_bearer_token(request):
    """
    Extract the OIDC ID token carried by the Authorization header
    """
    try:
...
class OIDCAuth(object):
    ...
    def post_form(self, url, form):
        """
        Post a form towards an endpoint published by an identity provider

        The response body is returned decoded also on error statuses, as the
        OAuth 2.0 error responses carry the error identifier in a JSON body.
        ...
        body = FileBodyProducer(BytesIO(urlencode(form).encode('utf-8')))

        response = yield State.get_agent().request(b'POST',
                                                   url.encode('utf-8'),
                                                   Headers(headers),
                                                   body).addTimeout(REQUEST_TIMEOUT, reactor)
    ...
    def validate_client(self, issuer, client_id):
        ...
        The token endpoint is probed with an invalid authorization code: an
        identity provider that recognizes the client rejects the grant with
        invalid_grant, while an unknown client or a client requiring
        credentials is rejected with invalid_client (RFC 6749 Section 5.2);
        a client requiring credentials would anyway be unusable by the
        client-side authorization code flow performed at login.
        """
        if not client_id:
    ...
    def verify_token(self, token, issuer, client_id):
        ...
        # so that a token minted for a resource cannot stand in for the ID
        ...
        # names the party it has been issued to, that party has to be the
        # configured client as well
        aud = claims.get('aud', [])
