...
@transact
def get_analyst_questionnaires(session, tid, language):
    """
    Get questionnaires available to analysts for custom metrics
    
    :param session: Database session
    :param tid: Tenant ID
    :param language: Language for serialization
...
class AnalystQuestionnairesCollection(BaseHandler):
    """
    Handler for analyst access to questionnaires
    """
    check_roles = 'analyst'
    
    def get(self):
        """
        Get all questionnaires available to the analyst
        """
        return get_analyst_questionnaires(self.request.tid, self.request.language)


@transact
def get_custom_metric_data(session, tid, questionnaire_id, field_id, option_ids, filters=None):
    """
    Calculate custom metric data based on questionnaire field responses
    
    :param session: Database session
    :param tid: Tenant ID
    :param questionnaire_id: ID of the questionnaire
    :param field_id: ID of the field to analyze
    :param option_ids: List of option IDs to track
    :param filters: Optional filters to apply
    :return: Dictionary with counts for each option
    """
    try:
        # Get all internal tips for this tenant with filters applied
        base_query = session.query(models.InternalTip).filter(models.InternalTip.tid == tid)
        base_query = apply_filters_to_query(base_query, filters)
        tips = base_query.all()
        
        print(f"[DEBUG] Custom metric query - Found {len(tips)} tips for tenant {tid}")
        print(f"[DEBUG] Looking for field_id: {field_id}, option_ids: {option_ids}")
        
        # Initialize counters for each option
        option_counts = {option_id: 0 for option_id in option_ids}
        tips_with_answers = 0
        
        # For each tip, check if the field response matches any of the tracked options
        for tip in tips:
            # Query the InternalTipAnswers for this tip's questionnaire responses
            tip_answers = session.query(models.InternalTipAnswers).filter(
                models.InternalTipAnswers.internaltip_id == tip.id
            ).first()
            
            if not tip_answers:
                print(f"[DEBUG] Tip {tip.id} has no InternalTipAnswers")
                continue
            
            # Use stat_answers for statistical analysis (not encrypted)
            # If stat_answers is empty, answers field contains encrypted data
            if tip_answers.stat_answers and isinstance(tip_answers.stat_answers, dict) and len(tip_answers.stat_answers) > 0:
                answers_dict = tip_answers.stat_answers
                print(f"[DEBUG] Using stat_answers for tip {tip.id}")
            elif tip_answers.answers and isinstance(tip_answers.answers, dict):
                answers_dict = tip_answers.answers
                print(f"[DEBUG] Using answers for tip {tip.id} (may be encrypted)")
            else:
                print(f"[DEBUG] Tip {tip.id} has no usable answers")
                continue
            
            tips_with_answers += 1
            
            # Debug: print all field IDs in this answer
            if tips_with_answers <= 2:  # Only log first 2 to avoid spam
                print(f"[DEBUG] Tip {tip.id} answer keys: {list(answers_dict.keys())}")
            
            # Check if our field_id is in the answers
            if field_id in answers_dict:
                answer_value = answers_dict[field_id]
                print(f"[DEBUG] Found field {field_id} with value: {answer_value} (type: {type(answer_value)})")
                
                # Handle different answer structures
                if isinstance(answer_value, dict):
                    # Handle case where answer is an object with 'value' property
                ...
                if isinstance(answer_value, list):
                    for value in answer_value:
                        ...
                        if value in option_ids:
                            ...
                            print(f"[DEBUG] Matched option: {value}")
                elif answer_value in option_ids:
                    option_counts[answer_value] += 1
                    print(f"[DEBUG] Matched option: {answer_value}")
            else:
                if tips_with_answers <= 2:
                    print(f"[DEBUG] Field {field_id} not found in answers")
        
        print(f"[DEBUG] Final counts: {option_counts}, tips with answers: {tips_with_answers}")
        
        return {
            'questionnaire_id': questionnaire_id,
            'field_id': field_id,
            'option_counts': option_counts,
            'total_submissions': len(tips),
            'tips_with_answers': tips_with_answers
        }
    except Exception as e:
        import traceback
        print(f"[ERROR] Error calculating custom metric: {e}")
...
class CustomMetricData(BaseHandler):
    """
    Handler for fetching custom metric data based on questionnaire responses
    """
    check_roles = 'analyst'
    ...
    def get(self):
        ...
        - option_ids: Comma-separated list of option IDs to track
        - Additional filter parameters (date_from, date_to, status, etc.)
        """
        # Helper function to get query parameter value
        def get_param(name):
            args = self.request.args.get(name.encode())
        ...
        questionnaire_id = get_param('questionnaire_id')
        field_id = get_param('field_id')
        option_ids_str = get_param('option_ids')
        
        if not questionnaire_id or not field_id or not option_ids_str:
            return {
                'error': 'Missing required parameters: questionnaire_id, field_id, option_ids'
            }
        
        option_ids = option_ids_str.split(',')
        
        # Parse filter parameters (same as Statistics handler)
        filters = {}
        status = get_param('status')
        if status:
            filters['status'] = status.split(',') if ',' in status else [status]
