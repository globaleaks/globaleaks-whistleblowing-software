...
def create_support_request(session, tid, user_session, mail_address, content, idp_claims=None):
    ...
    if author is not None:
        # Sealed to the session key when there is one, otherwise to the key of the account the
        # presented identity is bound to
    ...
    # Always sealed to the root tenant key: the scope decides who handles it, evaluated there
...
class SupportHandler(BaseHandler):
    ...
    def post(self):
        ...
        # Authenticated on the identity provider but without a session yet: the identity is bound to
        # an account and identifies it
