...
from globaleaks.utils.antivirus import enqueue_antivirus_scan, get_av_result, prepare_file_download
...
class WhistleblowerFileDownload(BaseHandler):
    ...
    def download_wbfile(self, session, tid, user_id, file_id):
        ...
        return ifile.name, ifile.id, itip.crypto_tip_prv_key, ifile.state, antivirus_enabled, recheck_needed

    @inlineCallbacks
    def get(self, wbfile_id):
        name, ifile_id, tip_prv_key, state, antivirus_enabled, recheck_needed = yield self.download_wbfile(
...
class ReceiverFileDownload(BaseHandler):
    ...
    def get(self, rfile_id):
        (name, filelocation, tip_prv_key, pgp_key,
         state, recheck_needed) = yield self.download_rfile(self.request.tid, rfile_id)
