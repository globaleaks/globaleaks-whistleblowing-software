...
def count_user_tip(session, accreditation_item):
    count_tip = (session.query(
        func.count(
            distinct(InternalTipForwarding.id)
        )).filter(InternalTipForwarding.oe_internaltip_id == accreditation_item.tid).one())
    count_user = (session.query(
        func.count(
            distinct(User.id)
        )).filter(User.tid == accreditation_item.tid).
                  filter(User.mail_address not in (accreditation_item.email, accreditation_item.admin_email)).one())
...
def get_all_accreditation(session):
    try:
        ...
        for accreditation_item in (session.query(Subscriber).filter(Subscriber.organization_name is not None)):
...
def get_accreditation_by_id(session, accreditation_id):
    try:
        request_accreditation = []
        accreditation_item = (session.query(Subscriber).filter(Subscriber.organization_name is not None).filter(
            Subscriber.sharing_id == accreditation_id).one()
        )
        t = (session.query(Tenant).filter(Tenant.id == accreditation_item.tid).one())
        count_tip, count_user = count_user_tip(session, accreditation_item)
        element = serialize_element(accreditation_item, count_tip, count_user, t)
...
class GetAccreditationHandler(BaseHandler):
    """
    This manager is responsible for receiving accreditation requests and forwarding them to the accreditation manager
    """
    check_roles = 'any'
