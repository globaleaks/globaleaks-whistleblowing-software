...
class MigrationScript(MigrationBase):
    ...
    def migrate_Comment(self):
        old_tenants = self.session_old.query(self.model_from['Comment']).all()
        new_comments = []
        for old_obj in old_tenants:
            new_comment = self.model_to['Comment']()
            for key in new_comment.__mapper__.column_attrs.keys():
                setattr(new_comment, key, getattr(old_obj, key, None))
                new_comment.hash_sha256 = sha256(getattr(old_obj, 'content'))
                new_comment.hash_sha512 = sha512(getattr(old_obj, 'content'))
