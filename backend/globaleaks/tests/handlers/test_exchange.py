...
@transact
def announced_mails(session):
    """
    Return the site and the address every mail waiting to be delivered is addressed to
    """
    return {(mail.tid, mail.address) for mail in session.query(models.Mail)}


@transact
def announce_new_content(session, state):
    """
    Run the announcement of what arrived, the sole part of the notification under test here
    """
    MailGenerator(state).announce_new_content(session, datetime_now(), [])


@transact
def announcements(session):
    """
    Return the site, the address and the subject of every mail waiting to be delivered
    """
    return {(mail.tid, mail.address, mail.subject) for mail in session.query(models.Mail)}
...
class ExchangeTest(helpers.TestHandlerWithPopulatedDB):
    ...
    def announce(self):
        """
        Return what the announcement of the content arrived writes to the two sites
        """
        waiting = yield announcements()
        yield announce_new_content(self.state)
        return (yield announcements()) - waiting
...
class TestCommunication(ExchangeTest):
    ...
    @inlineCallbacks
    def test_a_communication_is_announced_to_who_receives_it_and_updates_who_sent_it(self):
        # the reports the site already holds are announced first, so that what follows is what the
        # exchange alone writes, and the colleague reads its own: a recipient that has not read is
        # never written to twice
        yield self.announce()
        yield self.read(self.origin_id, user_id=self.dummy_receiver_2['id'])
...
class TestRequestAndTransmission(ExchangeTest):
    ...
    @inlineCallbacks
    def test_the_decision_is_announced_to_who_asked_and_to_the_colleagues_of_who_decided(self):
        colleague = yield create_user(2, None, self.get_dummy_receiver('colleague'), 'en')
        yield give_dummy_keys(colleague['id'])
        yield take_part(self.exchange['channel'], colleague['id'])
        yield db.refresh_tenant_cache()

        _, request = yield self.file()

        waiting = yield announced_mails()
        yield self.decide(request['id'], True)
        announced = (yield announced_mails()) - waiting

        # each side is announced on its own site, and the recipient that decided is left out
        self.assertEqual(announced, {(1, self.transmitter['mail_address']),
                                     (2, colleague['mail_address'])})

    @inlineCallbacks
    def test_a_request_and_a_transmission_are_announced_to_the_site_that_receives_them(self):
        # the reports the site already holds are announced first, so that what follows is what the
        # exchange alone writes
        yield self.announce()
