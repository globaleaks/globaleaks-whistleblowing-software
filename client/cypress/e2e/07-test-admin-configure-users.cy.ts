...
describe("admin add, configure, and delete users", () => {
  ...
  it("should add new users and profiles", () => {
    ...
    const make_profile = (profile: {name: string; value: string}) => {
    ...
    new_profiles.forEach((profile, i) => {
    ...
    const make_account = (user: {name: string; value: string; address: string}) => {
    ...
    new_users.forEach((user, i) => {
      make_account(user);
      cy.get(".userList").should('have.length', i+2);
    });
  });

  // The details of an account are written through its editor and read back after it is closed
  it("should edit the details of a user", () => {
    cy.login_admin();
    cy.openAdminUsers();
...
describe("Multiple role profile", () => {
  ...
  it("should require password change upon successful authentication", () => {
    ...
    cy.task<Record<string, string>>("getUsersPasswords").then((users_passwords) => {
