...
describe("admin enable antivirus", () => {
  it("should enable antivirus", () => {
    ...
    cy.get('[data-cy="antivirus-enabled"]').click();
    ...
    cy.get('[data-cy="save-antivirus-settings"]').click();
    cy.get('[data-cy="antivirus"]').click().should('be.visible').click();

    cy.get('[data-cy="antivirus-enabled"]').should("be.visible").should("be.checked");
    cy.logout();
    cy.waitForUrl("/#/login")
  });
});
